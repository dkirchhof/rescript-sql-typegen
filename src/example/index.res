...
/* ->QB1.select(a => (c(a.id), c(a.name))) */
/* ->QB1.where(a => Expr.eq(c2(a.name), v2("1"))) */

/* test->Utils.inspect */
/* test->QB1.toSQL->Js.log */
...
/* /1* // get avg song duration *1/ */
/* /1* QB.from(Db.SongsTable.t) *1/ */
/* /1* /2* ->QB1.select(s => column(s.duration)) *2/ *1/ */
/* /1* /2* ->QB1.select(s => Agg.avg(s.id)) *2/ *1/ */
/* /1* ->QB1.select(s => Agg.avg(1)) *1/ */
/* /1* ->QB1.toSQL *1/ */
/* /1* ->Js.log *1/ */
...
->QB_MANY.leftJoin2(Db.SongsTable.t, "songs", (_ar, al, s) => Expr.eq(c2(s.albumId), c2(al.id)))
